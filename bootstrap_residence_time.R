...
FILE_A <- "condition_A.csv"   # e.g. DMSO / untreated
FILE_B <- "condition_B.csv"   # e.g. dBET-1 / MZ-1 treated
...
#' Estimate starting parameters for a given survival curve via grid search.
#' Tries a grid of (alpha, ks, kns) combinations and returns the one with
#' the lowest RSS — used to initialise nlsLM per condition.
estimate_starts <- function(surv_df) {
  t    <- surv_df$t
  surv <- surv_df$surv

  alpha_grid <- seq(0.1, 0.9, by = 0.2)
  ks_grid    <- c(0.05, 0.1, 0.2, 0.5, 1.0, 2.0)
  kns_grid   <- c(0.1,  0.3, 0.7, 1.5, 3.0, 5.0)

  best_rss <- Inf
  best     <- c(alpha = 0.3, ks = 0.2, kns = 0.7)

  for (a in alpha_grid) {
    for (ks in ks_grid) {
      for (kns in kns_grid) {
        if (ks >= kns) next   # ks should be slower (smaller) than kns
...
#' Automatically estimates per-condition starting parameters via grid search.
#' Returns a named numeric vector c(alpha, ks, kns) or NAs on failure.
fit_two_component <- function(surv_df, starts = NULL) {
  if (is.null(starts)) starts <- estimate_starts(surv_df)

  tryCatch({
    fit <- nlsLM(
      surv ~ alpha * exp(-ks * t) + (1 - alpha) * exp(-kns * t),
      data    = surv_df,
      start   = as.list(starts),
    ...
    c(alpha = NA_real_, ks = NA_real_, kns = NA_real_)
  })
}
...
#' Starting parameters are estimated once from the observed data and
#' reused for all replicates (fast), with per-replicate fallback on failure.
#' Returns a B x 3 matrix of fitted parameters.
bootstrap_fit <- function(times, B = 2000, starts = NULL) {
  n      <- length(times)
  result <- matrix(NA_real_, nrow = B, ncol = 3,
                   dimnames = list(NULL, c("alpha", "ks", "kns")))

  # Estimate starts once from the full observed data if not provided
  if (is.null(starts)) {
    starts <- estimate_starts(make_survival(times))
    message(sprintf("  Auto-estimated starts: alpha=%.3f  ks=%.4f  kns=%.4f",
                    starts["alpha"], starts["ks"], starts["kns"]))
  }
  ...
  for (i in seq_len(B)) {
    ...
    params       <- fit_two_component(surv_df, starts)
    result[i, ]  <- params
...
cat(sprintf("%-12s  alpha = %.4f  ks = %.4f  kns = %.4f\n",
